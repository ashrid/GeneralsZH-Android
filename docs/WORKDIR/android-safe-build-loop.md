...
1. **Clean tree (tracked):** `git status --porcelain --untracked-files=no` empty (enforces commit-gating on tracked source; untracked files like scratch notes are tolerated so they don't block builds — the engine's CMake uses explicit source lists, not globs). Print the HEAD short-hash that this build will be named after. *Preflight first resets the DXVK submodule (`git -C references/fbraz3-dxvk checkout -- .`) because `cmake/dx8.cmake` applies `Patches/dxvk-android.patch` at configure time, dirtying the submodule after every build; the next configure re-applies the patch idempotently, so discarding the applied state here is safe. No-op if the submodule is uninitialized.*
