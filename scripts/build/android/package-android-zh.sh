...
# (meta.json is never read by the engine) and is tracked in the repo so builds are
# self-contained — no build-time download, no external dependency to maintain. To
# upgrade the driver, replace this .so with a newer Turnip build (keep the name).
