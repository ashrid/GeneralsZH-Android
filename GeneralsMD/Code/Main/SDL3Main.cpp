...
int main(int argc, char* argv[])
{
	...
	// (no sound — logcat: "Created device ..., No Output"). Force the opensl (OpenSL ES) backend
	// via ALSOFT_DRIVERS (per OpenAL Soft docs/env-vars.txt: "overrides the drivers config option").
	// Audio file loading is still skipped (OpenALAudioCache.cpp) to avoid the VmSize 19GB OOM —
	// both this backend fix and streaming are needed for real audio.
	setenv("ALSOFT_DRIVERS", "opensl", 0);
	{
		// GeneralsX @feature android-port 06/07/2026
